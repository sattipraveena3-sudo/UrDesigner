...
import {Design,Side,baseLayer,silhouettePath} from '@/lib/urdesigner/model';
import {paint,updateFromObject} from '@/lib/urdesigner/canvas';
export default function DesignCanvas({design,side,tool,brushColor,brushSize,selected,onSelect,onChange,onError,showGuides=true}:{design:Design;side:Side;tool:string;brushColor:string;brushSize:number;selected:string|null;onSelect:(id:string|null)=>void;onChange:(d:Design)=>void;onError:(s:string)=>void;showGuides?:boolean}){
 const guide=useRef<HTMLCanvasElement>(null);
 useEffect(()=>{const ctx=guide.current?.getContext('2d');if(!ctx)return;ctx.clearRect(0,0,512,640);if(!showGuides)return;const path=new Path2D(silhouettePath(design,side));ctx.lineJoin='round';ctx.strokeStyle='#ffffff';ctx.lineWidth=6;ctx.stroke(path);ctx.strokeStyle='#252d3d';ctx.lineWidth=2.4;ctx.stroke(path)},[design,side,showGuides]);
 ...
 return <div className="fabric-wrap"><canvas ref={element} aria-label="Editable garment artwork canvas. Use the layer controls to edit with the keyboard."/><canvas ref={guide} className="cut-guide-overlay" width={512} height={640} aria-hidden="true"/></div>
}
